...
# Apache Tomcat 8 CIS Benchmark v1.1.0 - Configuration Assessment Script (Sections 1 to 4)
# This script does NOT enforce settings — it audits and reports:
...
check_controls_v8() {
  local dir="$1"
  echo "───────────────────────────────────────────────"
  echo "🔍 Running Apache Tomcat 8 CIS Benchmark Checks"
  echo "───────────────────────────────────────────────"
  REPORT="$dir/tomcat8_cis_assessment_report.txt"
  echo "Apache Tomcat 8 Configuration Assessment - $(date)" > "$REPORT"
